...
if (isProduction && (!upstashUrl || !upstashToken)) {
  // Log which env names are present (but never log secret values).
  console.error('lib/redis: production mode but Upstash env missing. env presence:', {
...
if (upstashUrl && upstashToken) {
  // We'll lazily import and initialize the Upstash client on first use to avoid
  // any side-effects or env parsing at module import time (some Upstash libs
  // inspect envs when imported). The `ensureUpstashClient` function below
  // handles initialization when needed.
}
